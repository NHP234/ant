...
| 2026-06-23 | Cập nhật `docs/RAG_SERVICE.md` khớp hiện trạng RAG: bỏ mô tả TF-IDF/vector-only cũ, chuẩn hóa luồng SentenceTransformer + Calibrated SVM, hybrid PostgreSQL lexical + ChromaDB semantic fallback, DeepSeek `deepseek-v4-flash`, `chat_history` và incremental ingest từng sách. Verification: rà `rg` không còn các cụm stale như `TF-IDF`, `Gemini`, `Vector Search` cũ. |
| 2026-06-23 | Bổ sung batch pickup cho tab quản lý đặt trước: ADMIN/LIBRARIAN có thể lập một phiếu mượn từ toàn bộ hold `ACTIVE` chưa hết hạn của một sinh viên qua `POST /api/holds/pickup`; frontend nhóm hold theo sinh viên, refresh hold/borrow-slip/dashboard sau khi lập phiếu, còn xác nhận từng hold vẫn giữ làm thao tác riêng lẻ. Verification: targeted backend tests, full `mvn test`, targeted frontend ESLint, `npm run build`, `git diff --check` pass. |
| 2026-06-23 | Sửa lỗi mojibake notification khi lập phiếu từ hold: chuẩn hóa chuỗi thông báo batch pickup về UTF-8 đúng, thêm assertion test cho title/message tiếng Việt và cleanup 3 notification đã lưu sai trong DB Docker local. Verification: `BorrowSlipCreationServiceTest`, `git diff --check` pass. |
| 2026-06-23 | Chuẩn bị cấu hình deploy DigitalOcean VPS: thêm `docker-compose.prod.yml`, Caddy reverse proxy HTTPS, `.env.production.example`, hướng dẫn backup/restore PostgreSQL + ChromaDB và đổi frontend production API base sang same-origin `/api`. |
| 2026-06-24 | Kiem tra RAG tren VPS sau deploy: sync lai ChromaDB khop PostgreSQL (15.009 vector), bo sung stopword `chu/de` de query kieu "chu de lego chima" khong miss lexical title/author match. Verification: targeted RAG tests pass. |
| 2026-06-24 | Tach Query Normalizer cho RAG BOOK_SEARCH: loai bo question frames nhu `lien quan toi`, `chu de ve`, `noi dung ve`, uu tien lexical search bang query da normalize truoc query goc va dung query gon hon cho vector fallback. Verification: RAG pytest 37 tests pass. |
| 2026-06-24 | Siet guardrail chatbot BOOK_SEARCH: khi retrieval khong tra ve `source_books`, RAG khong goi LLM de tranh goi y sach khong co card/source; tra thong bao chua tim thay va goi y hoi lai bang tu khoa ngan hon. Verification: targeted chat context test pass. |
| 2026-06-24 | Sua lexical false positive cho query tieng Viet bo dau: giu token `ban` trong cum nghia nhu `bong ban`/`ban hang`, nhung normalizer loai dai tu `ban` dau cau de tranh nhieu; case `tim sach ve bong ban` khong con hien card sach chi match `bong`. Verification: RAG pytest 40 tests pass. |
| 2026-06-24 | Them answer-source consistency guard cho chatbot: neu LLM tra loi kieu khong tim thay sach phu hop du retrieval co `source_books`, RAG se xoa `source_books` truoc khi tra ve frontend de tranh UI hien card mau thuan voi cau tra loi. Verification: targeted RAG tests pass. |
| 2026-06-24 | Khac phuc tim kiem semantic loi voi cau hoi tieng Viet co dau/hoa/phu dinh: chia Query Normalizer thanh 3 truong (`original`, `normalized` co dau/casing, `lexical` khong dau/lowercase); ChromaDB dung `normalized` va Postgres dung `lexical`; giu lai tu phu dinh `khong` giua cau; ha threshold vector ve 0.53 de goi y dung cac sach nhu *Magical Toys* tu query cot truyen khong ten nhan vat. Verification: RAG pytest 43 tests pass. |
| 2026-06-25 | Nâng cấp RAG Service lên GPU CUDA: chuyển base image sang PyTorch CUDA, cấu hình device nvidia trong docker-compose. Tích hợp HF_HOME cache volume bền vững tránh tải lại mô hình bge-m3. Xây dựng cơ chế tự phục hồi VRAM OOM (Self-Healing Loop) tự động giảm batch size hoặc fallback CPU khi gặp mô tả sách siêu dài. |
| 2026-06-25 | Nâng cấp RAG search sang thuật toán Reciprocal Rank Fusion (RRF) kết hợp PostgreSQL Lexical và ChromaDB Semantic search với hằng số k=60; tối ưu hóa hiệu năng đồng bộ hóa (chỉ mã hóa sách mới chưa có vector trong ChromaDB), đồng bộ thành công toàn bộ 15.009 đầu sách. |
| 2026-06-25 | Bổ sung Flyway migration V14 tích hợp categories (weight C) và mô tả (weight D) vào search vector cho Full-Text Search trong PostgreSQL. |
...
| 2026-06-27 | Rà soát và đồng bộ tài liệu RAG sau nâng cấp BGE-M3/RRF: xác nhận DigitalOcean Droplet thường chạy CPU, bổ sung `HF_HOME` cho production compose, ghi rõ cần rebuild Chroma khi đổi embedding model, sửa mô tả semantic retrieval/RRF/LLM query rewrite cho khớp code. Thêm Flyway V15 để refresh `search_vector` khi đổi tên category và bổ sung normalizer cho query dạng "có nội dung ..." không có từ "về". |
| 2026-06-27 | Hoan thien deploy VPS tu snapshot local: restore PostgreSQL local len DigitalOcean, thay Chroma volume bang archive local da ingest san (khong full ingest tren VPS), rebuild stack production va smoke test frontend/API/RAG. Sua guard RAG de nhan dien cau tra loi "khong tim thay sach" ca khi text cu bi mojibake, tranh frontend hien source book card mau thuan voi cau tra loi. |
| 2026-06-28 | Rà soát và sửa mojibake trong RAG service: chuẩn hóa UTF-8 cho prompts, query normalizer, chat router và các test chat/LLM liên quan; xác nhận không còn pattern mã hóa hỏng trong `rag-service/app` và `rag-service/tests`. Verification: RAG `pytest` 40 tests pass, `git diff --check` pass. |
| 2026-06-28 | Cleaned legacy Playwright E2E artifacts from local PostgreSQL/ChromaDB and added Playwright global setup/teardown cleanup for test users, books, copies, holds, slips, notifications, audit logs, categories, authors, and RAG vectors. Hardened E2E test data naming with `e2e_`/`E2E` prefixes, fixed unsafe book-management selectors, and made API helpers fail fast on backend errors. Verification: frontend build pass, targeted `book-mgmt.spec.ts` pass, post-test DB artifact counts remain 0. |
| 2026-06-28 | Extended Playwright cleanup to include legacy `Duplicate Counter Book%` artifacts missed by the first cleanup pass, removed remaining local duplicate counter books from PostgreSQL/ChromaDB, and cleared stale Redis book detail cache. Verification: direct backend search for `Duplicate Counter Book 1780670604750` returns 0 results. |
| 2026-06-28 | Re-audited all Playwright E2E specs for data-producing patterns and expanded cleanup to cover historical mock-artifact names (`NFC Assignment Test Book`, batch/kiosk fixture books, `NFC Test Author`). Fixed `role-navigation` test ISBN generation to stay within backend validation. Verification: targeted `role-navigation.spec.ts` pass; PostgreSQL, Redis, and ChromaDB all report 0 matching E2E artifacts after teardown. |
| 2026-06-28 | Added API-based demo data seeding for defense rehearsals: `scripts/seed_demo_users_and_holds.py` creates idempotent demo librarians/students and two active holds per demo student through real backend APIs instead of Flyway/direct SQL, with documentation in `scripts/README_DEMO_DATA.md` and `docs/DEPLOYMENT.md`. |
| 2026-06-29 | Added `docs/test/TEST_CASE_SUMMARY.md` summarizing current backend service/controller/integration tests, frontend Playwright E2E specs, and RAG pytest suites for the thesis testing section. |
