...
function BorrowCard({ record }: { record: BorrowRecord }) {
  ...
  if (isOverdue) progressColor = "bg-red-500"
        ...
        <div className="flex-1 flex flex-col p-5">
          <div className="flex justify-between items-start gap-4">
            <div>
              <Link to={`/books/${record.bookId}`} className="hover:underline decoration-primary underline-offset-2">
                <h3 className="font-heading font-semibold text-lg line-clamp-1">{record.bookTitle}</h3>
              </Link>
              <p className="text-muted-foreground text-sm mt-1">{record.bookAuthor}</p>
            </div>
            <Badge variant={statusColors[record.status]} className={isOverdue ? 'bg-red-600' : 'bg-emerald-600'}>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6 text-sm">
            ...
            <div>
              ...
              <div className={`font-medium flex items-center gap-1.5 ${isOverdue ? 'text-red-600' : ''}`}>
          ...
          <div className="mt-auto pt-6">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className={`text-sm font-bold flex items-center gap-1 ${isOverdue ? 'text-red-600' : isWarning ? 'text-amber-600' : 'text-emerald-600'}`}>
